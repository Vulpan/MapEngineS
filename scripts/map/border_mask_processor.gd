...
func load_border_mask_from_image(src_image: Image, map_size: Vector2i, threshold: float = 0.5, spacing: float = 20.0) -> bool:
	...
	border_points.clear()
	border_pairs.clear()
	_extract_edge_pairs(spacing, 3.0)

	return true


func _extract_edge_points(threshold: float, spacing: float) -> void:
	var t := clampi(int(threshold * 255.0), 0, 255)

	# krok 1: piksele krawędziowe (ciemny z jasnym sąsiadem)
...
func _extract_edge_pairs(spacing: float, offset: float = 3.0) -> void:
	...
	for y in range(h):
		...
		for x in range(w):
			...
			var normal := _get_border_normal(x, y)
...
func is_border_pixel(pos: Vector2, threshold: float = 0.5) -> bool:
	...
	var x := clampi(int(round(pos.x)), 0, _w - 1)
	var y := clampi(int(round(pos.y)), 0, _h - 1)
	var t := clampi(int(threshold * 255.0), 0, 255)

	return _lum[y * _w + x] < t


func _is_dark(x: int, y: int, t: int) -> bool:
	return _lum[y * _w + x] < t


func _has_light_neighbor(x: int, y: int, t: int) -> bool:
	var w := _w
	var h := _h
	var lum := _lum
...
func _get_border_normal(x: int, y: int) -> Vector2:
